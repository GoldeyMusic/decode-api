...
// rows   = [{ label, value, link? }]                — métadonnées courtes en table label/value
// blocks = [{ label, body }]                        — paragraphes longs (verbatims feedback)
//                                                     label en uppercase muted + body multi-ligne
function renderOpsEmail({ title, intro, rows, blocks, footerLink }) {
  const rowsHtml = (rows || [])
    ...
    .join('');
  // Verbatims libres : label mono uppercase + bloc body avec retours à la ligne préservés.
  // On garde un padding latéral aligné sur les rows pour homogénéité.
  const blocksHtml = (blocks || [])
    .filter((b) => b && b.body !== null && b.body !== undefined && String(b.body).trim() !== '')
    .map((b) => `
      <div style="padding:14px 24px;border-top:1px solid #f0f0f0">
        <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#888;margin-bottom:6px">${escapeHtml(b.label)}</div>
        <div style="font-size:14px;color:#222;white-space:pre-wrap;line-height:1.5">${escapeHtml(String(b.body))}</div>
      </div>`)
    .join('');
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #eaeaea;border-radius:10px;overflow:hidden">
    ...
    ${blocksHtml}
    <div style="padding:${blocksHtml ? '16px' : '0'} 24px 20px">${footer}</div>
  </div>
</body></html>`;
}
