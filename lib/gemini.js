...
function buildPrompt(mode, title, fadrData) {
  const bpm = fadrData?.bpm || 'inconnu';
  const key = fadrData?.key || 'inconnue';
  const lufs = fadrData?.lufs ? `${fadrData.lufs} LUFS` : 'non mesuré';
  const duration = fadrData?.duration ? `${Math.round(fadrData.duration)}s` : 'inconnue';
...
Contexte mesure: BPM ${bpm}, tonalite ${key}, duree ${duration}, loudness ${lufs}.
...
5. Tes observations doivent refleter ce que TU entends, pas des generalites deductibles du BPM ou de la tonalite.
6. Termine par un verdict franc dans "potentiel": est-ce pret, presque pret, ou encore du travail, et dans quelle direction.
...
async function analyzeListening(fileBuffer, mimeType, title, artist, fadrData, mode) {
  const prompt = buildPrompt(mode, title || 'ce morceau', fadrData);
