...
//   asset.stems = [{ _id, name, type?, ... }, ...]
//   où name est typiquement "vocals", "drums", "bass", "other".
// Pour récupérer l'audio : POST /assets/download/{stemId} renvoie
//   { url: 'https://s3...presigned' }.
// On garde aussi un fallback sur stem.audioUrl/stem.url si jamais le
// shape Fadr évolue ou si la version "asset complet" embarque l'URL
// signée directement (cas observé sur certains plans).
//
...
async function downloadStems(asset, opts = {}) {
  ...
  const fetchSignedUrl = async (stem) => {
    ...
    // Fallback : POST signed-url style (certains tenants Fadr l'utilisent)
    try {
  ...
  const downloadOne = async (stem) => {
    const label = stem.name || stem.stemType || 'unknown';
  ...
  // Téléchargement en parallèle de tous les stems
