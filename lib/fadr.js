...
async function analyzeFile(buffer, filename, extension, mimeType, onProgress) {
  ...
  // Fetch final asset — wait extra for BPM/key to be populated
  await new Promise(r => setTimeout(r, 5000));
  ...
  // Log ALL fields to find bpm/key location
  console.log('[fadr] FULL ASSET:', JSON.stringify(finalAsset, null, 2));
...
function extractFadrData(task) {
  ...
  return {
    bpm: a.bpm || meta.bpm || meta.tempo || null,
    key: a.musicalKey || a.keySignature || meta.key || meta.musicalKey || null,
    lufs: a.lufs || meta.lufs || null,
    stems: a.stems || [],
    midi: a.midi || [],
    chords: a.chords || meta.chords || [],
    duration: a.duration || meta.length || null,
  };
}
