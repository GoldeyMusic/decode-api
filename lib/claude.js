...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics) {
  ...
  const mLufs = (typeof m.lufs === 'number') ? m.lufs : (typeof m.lufs === 'string' && m.lufs.trim() ? m.lufs : null);
  const hasMetrics = !!(mBpm || mKey || mLufs);
  if (hasMetrics) {
    console.log('[claude] fadr metrics injected in prompt — bpm:', mBpm, 'key:', mKey, 'lufs:', mLufs);
  }

  // Bloc MESURES OBJECTIVES (DSP 1.2) injecte au debut du systemPrompt si
  // on dispose de donnees Fadr. Format pensé pour aider Claude :
  // - liste claire de ce qu il SAIT (pour pouvoir le citer)
  // - liste explicite de ce qu il IGNORE encore (pour ne pas inventer)
  const dspBlock = hasMetrics ? `

MESURES OBJECTIVES DU MORCEAU (Fadr, conformes aux standards de mesure audio) :
${mBpm ? `- BPM : ${mBpm}` : ''}
${mKey ? `- Tonalité : ${mKey} (notation Fadr — tu peux reformuler en clair pour l artiste, ex: "G:maj" -> "Sol majeur", "Am" -> "La mineur").` : ''}
${mLufs ? `- Loudness intégré : ${mLufs} LUFS (ITU-R BS.1770).` : ''}

EXPLOITATION DE CES MESURES :
- Tu PEUX et tu DOIS citer ces valeurs dans les champs "why", "summary" ou "how" quand elles eclairent un point. Exemples : "Le BPM mesuré (${mBpm || 'N/A'}) confirme un tempo medium qui supporte bien la respiration des arrangements" ; "Avec un loudness de ${mLufs || 'N/A'} LUFS, le master est ${mLufs && mLufs < -12 ? 'sous les standards streaming (vise -10 a -12 LUFS pour Spotify/Apple)' : mLufs && mLufs > -8 ? 'tres pousse, attention aux services de normalisation' : 'dans la cible streaming'}".
- En MASTER & LOUDNESS, exploite la valeur LUFS pour calibrer ton diagnostic : sous -14 = trop sage, -8 a -10 = sweet spot streaming, au-dessus de -7 = limite de l'overcompression. Tes recettes "how" peuvent referencer la cible (ex: "remonter de 2-3 dB pour atteindre -10 LUFS").
...
- Le BPM peut justifier des recettes de delay/reverb time (ex: "delay 1/8 = ${mBpm ? Math.round(60000 / mBpm / 2) + ' ms' : '...'}"). Tu peux faire ces calculs.
...
- Les mesures listees doivent etre CITEES TEXTUELLEMENT, pas paraphrasees vaguement (dis "${mBpm || 'X'} BPM", pas "un tempo modéré de l ordre de 78"). Cela renforce la confiance de l artiste.
...
${hasPM ? '2. Des EXTRAITS DE COURS PUREMIX (transcripts d ingenieurs de mix reels) pertinents pour les problematiques evoquees par l ecoute. Tu peux t en inspirer pour ta pensee et ton vocabulaire, mais ne les cite jamais verbatim et ne les attribue pas nominativement.' : ''}${hasMetrics ? '\n3. Des MESURES OBJECTIVES (BPM, tonalite, LUFS) calculees sur le fichier audio par un module DSP fiable. Ce sont des verites factuelles a utiliser dans la fiche.' : ''}${intentBlock}${dspBlock}
...
INTENTION DECLAREE: "${intentStr}"` : ''}${hasMetrics ? `
MESURES OBJECTIVES (Fadr) :${mBpm ? ` BPM=${mBpm}` : ''}${mKey ? ` · Tonalite=${mKey}` : ''}${mLufs ? ` · LUFS=${mLufs}` : ''}
↳ Cite ces valeurs textuellement dans tes "why"/"summary"/"how" quand elles eclairent un point. N invente AUCUNE autre mesure.` : ''}
