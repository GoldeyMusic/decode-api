...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics) {
  ...
  // - verdicts pre-calcules pour calibrer le diagnostic master
...
MESURES OBJECTIVES :${mBpm ? ` BPM=${mBpm}` : ''}${mKey ? ` · Tonalite=${mKey}` : ''}${mLufs != null ? ` · LUFS=${mLufs}` : ''}${mLra != null ? ` · LRA=${mLra} LU` : ''}${mTruePeak != null ? ` · TruePeak=${mTruePeak} dBTP` : ''}
↳ Cite ces valeurs textuellement dans tes "why"/"summary"/"how" quand elles eclairent un point. En MASTER, calibre le diagnostic sur LUFS/LRA/TruePeak. N invente AUCUNE autre mesure.` : ''}
