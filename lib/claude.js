...
// ─────────────────────────────────────────────────────────────
// Ticket 4.2 — Advice-followed locking
//
// Quand l utilisateur a coche des items comme "implementes" en V_(n-1)
// puis uploade une V_n, on verifie :
//  - L item correspondant a-t-il disparu en V_n  → advice suivi (resolu)
//  - L item est encore present mais avec un score >= V_(n-1)         → advice suivi (confirme)
//  - L item est encore present avec un score < V_(n-1)                → advice non suivi (signale)
//
// Lock : pour les items "non suivis", on ramene le score V_n a celui de
// V_(n-1) afin que la baisse ne penalise pas l utilisateur — il a pris
// le temps de cocher, on ne lui inflige pas une regression supplementaire.
//
// Le matching V_(n-1) ↔ V_n se fait d abord par id ; si l id n existe pas
// en V_n, on tente une similarite Jaccard sur les titres dans la meme
// categorie (seuil 0.4). Pas de match → on considere que le probleme a
// disparu, donc advice suivi.
// ─────────────────────────────────────────────────────────────
function _norm(s) {
...
function _jaccard(a, b) {
  const A = new Set(_norm(a));
  const B = new Set(_norm(b));
  if (A.size === 0 || B.size === 0) return 0;
  let inter = 0;
  for (const w of A) if (B.has(w)) inter++;
  return inter / (A.size + B.size - inter);
}
function _findItemById(fiche, id) {
  for (const el of fiche?.elements || []) {
    for (const it of el?.items || []) {
      if (it && it.id === id) return { item: it, cat: el.cat };
    }
  }
  return null;
}
function _findSimilarItem(fiche, targetTitle, targetCat) {
  let best = null;
  for (const el of fiche?.elements || []) {
    if (el?.cat !== targetCat) continue;
    for (const it of el?.items || []) {
      const sim = _jaccard(it?.title, targetTitle);
      if (sim >= 0.4 && (!best || sim > best.sim)) best = { item: it, cat: el.cat, sim };
    }
  }
  return best;
}

function applyAdviceCheck(currentFiche, previousFiche, completedIds) {
  if (!currentFiche || !previousFiche || !Array.isArray(completedIds) || completedIds.length === 0) {
    return currentFiche;
  }
  ...
  for (const id of completedIds) {
    ...
    const prev = _findItemById(previousFiche, id);
    if (!prev || !prev.item) continue;
    const prevItem = prev.item;
    const prevCat = prev.cat;
    const oldScore = typeof prevItem.score === 'number' ? prevItem.score : null;

    // 1) Match exact par id en V_n
    let match = _findItemById(currentFiche, id);
    if (!match) {
      // 2) Fuzzy par titre dans la meme categorie
      match = _findSimilarItem(currentFiche, prevItem.title, prevCat);
    }

    if (!match || !match.item) {
      followed.push({ title: prevItem.title || '', cat: prevCat || '', status: 'resolved' });
      continue;
    }

    const newScore = typeof match.item.score === 'number' ? match.item.score : null;
    if (oldScore == null || newScore == null || newScore >= oldScore) {
      followed.push({
        title: prevItem.title || '', cat: prevCat || '',
        status: 'confirmed', oldScore, newScore,
      });
    } else {
      // SCORE LOCK : on bloque le score V_n a oldScore pour ne pas penaliser
      // l utilisateur d avoir coche. On signale le desaccord cote front.
      match.item.score = oldScore;
      match.item.advice_locked = true;
      lockedCats.add(prevCat || '');
      unfollowed.push({
        title: prevItem.title || '', cat: prevCat || '',
        newTitle: match.item.title || '',
        oldScore, newScore,
      });
    }
  }

  if (followed.length || unfollowed.length) {
    currentFiche.advice_check = {
      followed,
      unfollowed,
      previousCompletedCount: completedIds.length,
      lockedCategories: Array.from(lockedCats).filter(Boolean),
    };
  }
  return currentFiche;
}
...
// ─────────────────────────────────────────────────────────────
// applyScoreFloor — Ticket 4.1
//
// Plafond de score quand des items priorite "high" restent non resolus.
// Reponse directe a "j ai uploade une V2 sans corriger mes vrais
// problemes mais le score grimpe quand meme". Le score global ne peut
// pas franchir un plafond tant que des chantiers high subsistent.
//
// Bareme :
//   1 item high  → ceiling 84
//   2 items high → ceiling 79
//   3+ items     → ceiling 74
//
// On expose `score_floor` au front pour que la fiche puisse afficher
// "Plafond applique : N item(s) prioritaire(s) a traiter".
// ─────────────────────────────────────────────────────────────
function applyScoreFloor(fiche) {
  ...
  for (const el of fiche.elements) {
    ...
    for (const it of el.items) {
      ...
      if (prio === 'high') {
        highItems.push({ id: it.id || null, title: it.title || '', cat: el.cat || '' });
      }
    }
  }

  if (highItems.length === 0) {
    fiche.score_floor = { highCount: 0, applied: false, ceiling: null, original: fiche.globalScore ?? null };
    return fiche;
  }

  const ceiling = highItems.length === 1 ? 84
                : highItems.length === 2 ? 79
                : 74;
  const original = typeof fiche.globalScore === 'number' ? fiche.globalScore : null;
  const willCap = original != null && original > ceiling;

  if (willCap) fiche.globalScore = ceiling;
  ...
  fiche.score_floor = {
    ...
    ceiling,
    original,
    items: highItems.map(h => h.title).filter(Boolean).slice(0, 5),
  };
...
// normalise les scores /100, et calcule un globalScore /100 pondere si absent.
...
// ─────────────────────────────────────────────────────────────
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletedItems) {
  ...
  // Ticket 4.2 — items que l utilisateur a coche comme "implementes" en V_(n-1).
  // Forme : [{ id, cat, title, score, why }]. Si fournis, on les expose dans le
  // prompt pour cadrer l ecoute, et on post-process applyAdviceCheck.
  const hasCompletions = Array.isArray(previousCompletedItems) && previousCompletedItems.length > 0;
  const completionsBlock = hasCompletions ? `

ITEMS QUE L UTILISATEUR AFFIRME AVOIR IMPLEMENTES (coches dans la checklist apres V_(n-1)):
${previousCompletedItems.map(it => `- [${it.cat || '?'}] "${(it.title || '').slice(0, 120)}"${typeof it.score === 'number' ? ` (score V_(n-1): ${it.score}/100)` : ''}`).join('\n')}

Pour CHACUN de ces items, verifie dans ton ecoute si le probleme est effectivement resolu. Si tu l entends encore, signale-le clairement dans la nouvelle fiche (item correspondant garde une priorite high). Si tu ne l entends plus, ne le mentionne plus comme defaut. Important : ne baisse PAS le score d un domaine ou l utilisateur a coche un item, sauf si tu entends une vraie regression au-dela de l item coche.
` : '';
...
- Ce qui ressemble a un defaut mais s inscrit dans l intention est un CHOIX ASSUME, pas une erreur. Ne propose pas de le "corriger", sauf si tu penses que le choix dessert l intention elle-meme (et dans ce cas, explique le conflit dans le "why" de l item ou le "target" de la tache plan).
...
${hasPM ? '2. Des EXTRAITS DE COURS PUREMIX (transcripts d ingenieurs de mix reels) pertinents pour les problematiques evoquees par l ecoute. Tu peux t en inspirer pour ta pensee et ton vocabulaire, mais ne les cite jamais verbatim et ne les attribue pas nominativement.' : ''}${intentBlock}${completionsBlock}
...
Rappel: categories vides autorisees (items: []). ${isRef ? '' : 'Plan minimaliste autorise si l ecoute est positive. Scores ancres dans le verdict de l ecoute.'} Reprends le ton et le verdict de l ecoute dans le summary.${hasPM ? ' Inspire-toi des extraits PureMix pour affiner la formulation, sans jamais les citer.' : ''}${hasIntent ? ' Tiens compte de l intention declaree : les choix qui en decoulent ne sont pas des defauts.' : ''}`;
  ...
  // Ticket 4.2 — verification des items coches comme implementes en V_(n-1).
  // Doit s appliquer AVANT le score-floor (il peut bumper des item.score, ce qui
  // fait remonter la moyenne, ce qui change le globalScore… qu on recalcule
  // ensuite en passant par applyScoreFloor → globalScore est deja fixe par
  // normalizeIds, donc on garde tel quel : on bumpe l item mais le globalScore
  // ne diminue pas. Cas simple, pas de recalcul.).
  const checked = applyAdviceCheck(normalized, previousFiche, (previousCompletedItems || []).map(it => it?.id).filter(Boolean));

  // Ticket 4.1 — applique le plafond de score (priorite high non resolue)
  // APRES la normalisation, pour que `globalScore` soit deja calcule sur /100.
  return applyScoreFloor(checked);
}
