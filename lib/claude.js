...
//     branchees en 1.1bis (DSP_PLAN). L injection effective dans le
//     systemPrompt est traitee en 1.2 (reformulation). Ici on accepte
//     le parametre pour pouvoir cabler le pipeline sans casser les
//     appels existants.
...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics) {
  ...
  const hasMetrics = !!(fadrMetrics && (fadrMetrics.bpm || fadrMetrics.key || fadrMetrics.lufs));
  if (hasMetrics) {
    console.log('[claude] fadr metrics available — bpm:', fadrMetrics.bpm, 'key:', fadrMetrics.key, 'lufs:', fadrMetrics.lufs, '(prompt injection branchee en 1.2)');
  }
