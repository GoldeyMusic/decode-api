...
function buildLocaleInstruction(locale) {
  ...
  if (norm === 'en') {
    return `[ABSOLUTE LANGUAGE CONSTRAINT] Even though the detailed instructions below are written in French, your ENTIRE JSON OUTPUT must be in ENGLISH. Produce ALL human-visible string values in English — every "label", "detail", "tools", "task", "daw", "metered", "target", "summary", "verdict", "tips" and any other natural-language field. The French instructions below are a style/structure guide; DO NOT echo them back in French.
...
- "verdict" and "summary" in particular MUST be in English, written as a native English-speaking sound engineer would.
...
// Rappel en fin de user prompt (recency bias) pour que Claude ne dérive pas
// vers le FR à cause des 40+ lignes d'instructions en français.
function buildLocaleReminder(locale) {
  const norm = (locale || '').toString().toLowerCase().slice(0, 2);
  if (norm === 'en') {
    return `\n\n=== FINAL REMINDER ===\nEvery natural-language string in the JSON MUST be in ENGLISH, including "verdict" and "summary". Write as a native English-speaking sound engineer would. Translate any French source material (the listening JSON, previous fiche…) before reusing its content.`;
  }
  return '';
}
...
async function chat(messages, system, opts = {}) {
  const maxTokens = typeof opts.maxTokens === 'number' ? opts.maxTokens : 800;
  const model = typeof opts.model === 'string' && opts.model ? opts.model : MODEL;
