...
async function generateFiche(mode, daw, title, artist, listening) {
  ...
  const systemPrompt = `Tu es ingenieur du son expert. Tu recois une ECOUTE qualitative du morceau faite par un collegue qui l a ecoute (JSON). C est ta SEULE source d information.
 
Ton role: transformer cette ecoute en fiche structuree pour ${daw}, ET ${isRef ? 'un set de conseils de reproduction' : 'un plan d actions concretes'}.
 
REGLES DE FER:
- L ecoute est ta SEULE source. Tu n as AUCUNE mesure technique (pas de BPM, pas de LUFS, pas de tonalite). Ne mentionne jamais de valeurs mesurees.
...
- ${!isRef ? 'Pour plan: "metered" = description qualitative de ce que l ecoute observe (ex: "bas-medium charge") ou "" si rien a observer ; "target" = direction cible formulee en mots (ex: "plus de clarte sur la voix") ; "task" = titre court + "daw" = instruction concrete pour ' + daw + ' en une phrase conversationnelle (pas de liste sterile de parametres). Priorite "p" = HIGH / MED / LOW selon ce que l ecoute considere comme critique.' : ''}
...
- Reponds UNIQUEMENT en JSON valide. Pas de markdown, pas de backticks.`;
  ...
  const prompt = `ECOUTE QUALITATIVE (source unique):
${listeningStr}
