...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche) {
  ...
  const persoTemplate = JSON.stringify({
    ...
    globalScore: 72,
    elements_detectes: { voix: ["voix lead"], drums: ["kick","caisse claire"], basses: ["basse"], instruments: ["piano","guitare clean"], spatial: ["reverb large sur voix"] },
    verdict: "...",
    summary: "..."
  });
...
` : ''}
- Champ "elements_detectes" (objet, racine) : liste EXHAUSTIVE des elements musicaux que l ecoute mentionne ou implique avoir entendus, regroupes par famille (voix, drums, basses, instruments, spatial). Inclut TOUT ce qui est present meme sans probleme a traiter (ex: "kick", "caisse claire", "choeurs", "pad", "guitare clean"). Ce champ sert UNIQUEMENT a comparer des versions entre elles (detecter ajouts/retraits) et n est jamais affiche. Si l ecoute ne mentionne pas un element, ne l invente pas. Famille absente = tableau vide [].
- Reponds UNIQUEMENT en JSON valide. Pas de markdown, pas de backticks.`;

  const calibrationStr = previousFiche ? "CALIBRAGE — VERSION PRECEDENTE DU MEME TITRE (anchor pour scores et elements_detectes) :\n" + JSON.stringify(previousFiche, null, 2) + "\n\nREGLES DE CALIBRAGE :\n- Garde des scores COHERENTS avec la version precedente (variation max +/-1 point) sauf si l ecoute mentionne explicitement une amelioration ou degradation sur cet element.\n- Ne fais varier un score de 2+ points QUE si l ecoute decrit un changement reel.\n- globalScore : ne s eloigne jamais de plus de 5 points du precedent SAUF si l ecoute decrit un changement majeur.\n- elements_detectes : reprends la meme liste que la version precedente, et ajoute/retire UNIQUEMENT ce que l ecoute confirme avoir entendu differemment.\n\n" : "";
