...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics, declaredGenre = null, genreUnknown = false, uploadType = 'master') {
  ...
  // Faux positif typique : un beatmaker amateur met Ozone/limiteur sur son
  // bus master des le debut du mix. Resultat : LUFS/LRA/TP/corr/mono compat
  // passent tous (gonfles par le limiteur), 6/6 criteres techniques OK. Si
  // l user coche "Master" par confusion (parce que son mix sonne "fort"),
  // le mode commercial s'activerait a tort sur un mix encore bancal.
  //
  // Garde-fou : on contre-verifie via l'ecoute Gemini. Un VRAI master
  // commercial professionnel ne genere pas plusieurs critiques qualitatives
  // chez Gemini. Si l'ecoute pointe 2+ items dans "a_travailler" ET qu'au
  // moins un mentionne un defaut structurel (boueux, ecrase, manque de
  // definition, masquage, sature, etc.), le mode commercial est DESACTIVE
  ...
  const STRUCTURAL_DEFECT_RE = /\b(boueux|boue|écrasé|ecrase|écrase|saturation|satur(?:é|e)|incohér|manque\s+de\s+(définition|definition|corps|présence|presence|punch|tranchant|clarté|clarte)|masqu(age|é|e)|déphas|dephas|résonance gênante|distorsion non voulue|fatigant|agressif|trop\s+(brillant|sombre|comprimé|comprime|en\s+avant|en\s+retrait))\b/i;
  const structuralDefects = aTravailler.filter((item) => {
    const txt = typeof item === 'string' ? item : (item?.observation || item?.text || JSON.stringify(item || ''));
    return STRUCTURAL_DEFECT_RE.test(txt);
  });
  const tooManyDefects = aTravailler.length >= 2 && structuralDefects.length >= 1;
  const isLikelyCommercialMaster = isMaster && commercialScore >= 5 && !tooManyDefects;
  if (isMaster && commercialScore >= 5) {
    console.log('[claude] commercial-master eval — score:', commercialScore + '/6', 'a_travailler items:', aTravailler.length, 'structural defects:', structuralDefects.length, 'activated:', isLikelyCommercialMaster);
  }
...
(6) Le summary doit reconnaitre la qualite commerciale du master. Formulations OBLIGATOIRES :
  ...
  - "Quelques verifications a faire avant publication" (UNIQUEMENT si True Peak ou mono compat objectif mentionne)
...
(7) Le globalScore final doit refleter cette realite : un master commercial honnete doit etre 88-95, pas 70-83. Calibre tes scores items en consequence — si tous les items qualitatifs sont a 85+ et seuls 1-2 items techniques DSP sont a 65-75, le globalScore tombe naturellement dans 88-92.
...
(2) Quand une MESURE DSP existe, elle impose la fourchette de score (et donc la severite) :
  ...
  - Mono compat 2-4 LU       -> score 72-80, priority "med" ou "low" selon le contexte (zone typique des masters commerciaux pop/folk-pop en stereo enveloppante)
  - Mono compat <= 2 LU      -> score 85+,   priority "low"
