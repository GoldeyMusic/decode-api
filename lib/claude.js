...
async function generateFiche(mode, daw, title, artist, listening, pmContext, previousFiche, intent, previousCompletions, fadrMetrics, declaredGenre = null, genreUnknown = false, uploadType = 'master') {
  ...
  if (mMonoCompat != null) {
    ...
    else monoCompatVerdict = 'mono a reprendre (perte ' + mMonoCompat + ' LU) : des elements en side s attenuent en lecture mono (telephone, enceinte BT) — important a corriger mais pas vital, le titre tient debout en stereo';
  }
  let balanceVerdict = '';
  if (mBal != null) {
  ...
  const commercialChecks = {
    lufsInTarget: (mLufs != null) && mLufs >= -14 && mLufs <= -7,
    lraReasonable: (mLra != null) && mLra >= 4 && mLra <= 14,
  ...
  const isLikelyCommercialMaster = isMaster && commercialScore >= 5;
  if (isLikelyCommercialMaster) {
    console.log('[claude] commercial-master detected — score:', commercialScore + '/6', 'checks:', commercialChecks);
  }
...
- SCORES : par defaut HAUTS (80-90) sur les items master/loudness — un mix non-masterise n est pas "rate" parce qu il n est pas a -10 LUFS. Score bas (sous 70) UNIQUEMENT en cas de probleme technique objectif qui fera echouer le mastering : clipping deja inscrit dans les samples, mono cassee (correlation < 0.2), desequilibre L/R majeur, distorsion non voulue audible, masquage frequentiel grave. Ces problemes-la, oui, doivent etre corriges AVANT d envoyer au mastering.
...
` : `
L artiste declare envoyer un MASTER pret pour streaming. Calibre la section MASTER & LOUDNESS au standard publication :
- Loudness, LRA et True Peak SONT des verdicts a plein. Si LUFS sous la cible streaming (-14 a -10 LUFS selon plateforme), si True Peak > -1 dBTP, si LRA tres ecrase ou trop large pour le genre, ce sont des items a corriger AVANT publication.
- Recettes "how" en cibles publication : "remonter le bus master de X dB pour atteindre -10 LUFS", "limiter ceiling -1 dBTP en sortie", "alleger la compression bus master pour regagner X LU de LRA".
- SCORES : standard streaming. Un master a -16 LUFS qui aurait du etre a -10 LUFS merite un score bas, parce qu il sera ecrase par la concurrence en lecture normalisee.
...
Les mesures techniques (LUFS, LRA, True Peak, correlation L/R, mono compat, sibilantes) confirment que ce titre est masterise au niveau d une release commerciale publiee. Score technique : ${commercialScore}/6 criteres remplis. Tu DOIS adapter ton diagnostic :

(1) Tu reconnais que les choix de mix sont des DECISIONS PROFESSIONNELLES ASSUMEES, pas des defauts a corriger. Une basse "ronde et chaleureuse" sur une ballade pop = parti pris artistique, pas "boueux". Une snare laidback sur un slow = signature stylistique, pas "en retrait". Une voix +5 LU au dessus de l instru sur du pop/soul = standard du genre, pas "trop forte".

(2) Tous les items sont des VALIDATIONS EXPLICITES (score 80+, priority "low") qui reconnaissent ce que l ecoute valide. Le "why" decrit positivement le choix observe. Le "how" se limite a "RAS, conserver tel quel" eventuellement complete par "surveiller en mastering que..." pour signaler les points fragiles si remaster.

(3) Un item MED (60-79) n est autorise QUE si une MESURE DSP OBJECTIVE le contredit factuellement (ex: True Peak > 0 dBTP, correlation inversee, masquage frequentiel mesure entre stems). PAS de MED sur des interpretations qualitatives "manque de definition", "pourrait gagner en", "un peu boueux" : sur un master commercial, ces formulations sont INTERDITES.

(4) Un item HIGH (≤ 59) est INTERDIT sauf defaut technique objectif et grave (clipping inscrit dans les samples, phase inversee, distorsion non voulue mesuree).

(5) Le summary doit reconnaitre la qualite commerciale du master ("titre au niveau professionnel publie", "production aboutie", etc.) sans flagornerie. Pas de "deux points a reprendre avant mastering" — c est deja mastere. Eventuellement : "quelques verifications en pre-publication" (mono, True Peak streaming), JAMAIS de "a corriger".

(6) Le globalScore final doit refleter cette realite : un master commercial honnete doit etre 88-95, pas 70-78. Calibre tes scores items en consequence.
...
(1) Score impose la severite (correspondance obligatoire, aucune exception) :
  - score >= 80         -> priority: "low"   (validation ou point mineur)
...
(2) Quand une MESURE DSP existe, elle impose la fourchette de score (et donc la severite) :
  - Mono compat > 2 LU       -> score 60-70, priority "med" (important a reprendre, mais pas vital — le titre tient en stereo)
  - Mono compat 1-2 LU       -> score 72-80, priority "med" ou "low" selon le contexte
  - Mono compat <= 1 LU      -> score 85+,   priority "low"
  ...
  - Delta voix/instru entre -3 et +3 LU -> "voix bien posee" : score 80+, priority "low". L item peut quand meme exister en MED si l ecoute pointe un probleme de TIMBRE (presence, congestion, brillance), mais JAMAIS un probleme de NIVEAU dans cette fourchette.
  - Delta voix/instru entre +3 et +8 LU -> classer SELON L ECOUTE, pas selon le seuil seul : si l ecoute valide la voix ("bien posee", "intelligible", "presente"), c est un CHOIX ARTISTIQUE legitime (typique pop/rock) : score 78-85, priority "low". MED 60-75 UNIQUEMENT si l ecoute signale explicitement un detachement perceptif (mots "decrochee", "detachee", "agressive", "artificielle"). PAS DE HIGH sur ce delta seul.
...
Rappel: ${isRef ? 'categories vides autorisees uniquement si "par_categorie" est vide ou absent.' : 'genere AU MOINS 1 item par categorie commentee dans "par_categorie" (validation si rien a corriger, correctif sinon). Une categorie ne reste vide que si "par_categorie" n a rien dit dessus (typiquement: voix sur un titre instrumental). Scores ancres dans le verdict de l ecoute.'} Reprends le ton et le verdict de l ecoute dans le summary.${hasPM ? ' Inspire-toi des extraits PureMix pour affiner la formulation, sans jamais les citer.' : ''}${hasIntent ? ' Tiens compte de l intention declaree : les choix qui en decoulent ne sont pas des defauts.' : ''}`;
  ...
  let normalized = applyScoreFloor(normalizeIds(parsed, uploadType), previousFiche);
  normalized = applyAdviceLock(normalized, previousFiche, previousCompletions);
  // Debug temporaire (a retirer apres validation) : expose la detection
  // master commercial dans la fiche pour pouvoir diagnostiquer sans logs.
  normalized._debug_commercial = {
    uploadType,
    score: commercialScore,
    checks: commercialChecks,
    activated: isLikelyCommercialMaster,
  };
  return normalized;
}
